(() => {
    ...
    (() => {
        ...
        tippy_esm("[tip-title]", {
            ...
            animation: "fade",
            content(reference) {
                const title = reference.getAttribute("tip-title");
                reference.removeAttribute("tip-title");
                return title;
            }
        });
        ...
        let visualizationSliders = document.querySelectorAll(".visualization__slider");
        let interleaveOffset = .5;
        function initSliders() {
            if (document.querySelector(".preview__slider")) new core(".preview__slider", {
                ...
                lazy: true,
                scrollbar: {
                    el: ".preview__scrollbar",
                    draggable: true
                },
                navigation: {
                    prevEl: ".preview__swiper-btn-prev",
                    nextEl: ".preview__swiper-btn-next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1
            ...
            if (visualizationSliders) visualizationSliders.forEach((item => {
                const swiper = new core(item, {
                    ...
                    scrollbar: {
                        el: item.querySelector(".visualization__slider-scrollbar"),
                        draggable: true,
                        hide: false
                    },
                    navigation: {
                        prevEl: item.querySelector(".visualization__slider-btn-prev"),
                        nextEl: item.querySelector(".visualization__slider-btn-next")
                    },
                    on: {
                        imagesReady: function(item) {
                            item.el.classList.remove("loading");
                            item.autoplay.start();
                        },
                        progress: function(item) {
                            for (let i = 0; i < item.slides.length; i++) {
                                let slideProgress = item.slides[i].progress;
                                let innerOffset = item.width * interleaveOffset;
                                let innerTranslate = slideProgress * innerOffset;
                                item.slides[i].querySelector(".visualization__slide-image").style.transform = "translateX(" + innerTranslate + "px)";
                            }
                        },
                        touchStart: function(item) {
                            for (let i = 0; i < item.slides.length; i++) item.slides[i].style.transition = "";
                        },
                        setTransition: function(item, speed) {
                            for (let i = 0; i < item.slides.length; i++) {
                                item.slides[i].style.transition = speed + "ms";
                                item.slides[i].querySelector(".visualization__slide-image").style.transition = speed + "ms";
                            }
                        }
                    },
                    breakpoints: {
                ...
                const slider = item.closest(".visualization__item-wrapper");
                const spollers = slider.querySelectorAll(".spollers__title");
                spollers.forEach(((item, x) => {
                    item.addEventListener("click", (e => {
                        const currentTitle = x;
                        swiper.activeIndex = currentTitle;
                        swiper.slideTo(x, 800);
                    }));
                }));
            }));
            ...
            let sliderWithCounter = document.querySelectorAll(".slider-count");
            if (sliderWithCounter) sliderWithCounter.forEach((i => {
                new core(i, {
                    modules: [ Navigation ],
                    observer: true,
                    observeParents: true,
                    ...
                    navigation: {
                        prevEl: i.nextElementSibling.querySelector(".slider-nav__button-prev"),
                        nextEl: i.nextElementSibling.querySelector(".slider-nav__button-next")
                    },
                    on: {}
                });
                const sliderSection = i.closest("section");
                const sliderCounter = sliderSection.querySelector(".slider-counter");
                const counter = i.parentNode.querySelector(".slider-counter") || sliderCounter;
                if (counter) {
                    const activeSlide = counter.querySelector(".slider-counter__current");
                    const allSlides = counter.querySelector(".slider-counter__all");
                    const visibleSlides = i.querySelectorAll(".swiper-slide-visible");
                    allSlides.innerHTML = i.swiper.slides.length;
                    activeSlide.innerHTML = i.swiper.activeIndex + visibleSlides.length;
                    i.swiper.on("slideChange", (function() {
                        activeSlide.innerHTML = visibleSlides.length + i.swiper.activeIndex;
                    }));
                }
            }));
        }
        ...
        window.addEventListener("DOMContentLoaded", (() => {
            const resizableSwiper = (breakpoint, swiperClass, swiperSettings, callback) => {
                ...
                const enableSwiper = function(className, settings) {
                    swiper = new core(className, settings);
            ...
            resizableSwiper("(max-width: 992px)", ".team__slider", {
                loop: true,
                spaceBetween: 20,
                slidesPerView: "auto",
                pagination: {
        ...
        if (tagsContainer) {
            ...
            const tagReset = tagsContainer.querySelector(".tag--reset");
            tags.forEach((item => {
                item.addEventListener("click", (e => {
                    if (!item.classList.contains("tag--reset")) e.target.classList.toggle("tag--active");
                }));
                tagReset.addEventListener("click", (e => {
        ...
        const offsetHeader = headerMenu.offsetHeight;
        const preview = document.querySelector(".preview");
        preview.style.paddingTop = offsetHeader + "px";
        if (headerMenu) headerLinks.forEach((item => {
            item.addEventListener("click", (e => {
                removeActive();
                let target = e.target;
                if (!target.classList.contains("menu__link--active")) target.classList.add("menu__link--active");
            }));
        }));
            ...
            entries.forEach((entry => {
                if (entry.isIntersecting) {
                    ...
                    menuLinks.forEach((item => {
                        if (item.getAttribute("data-goto").replace(".", "") === entry.target.id) item.classList.add("menu__link--active"); else item.classList.remove("menu__link--active");
                    }));
                }
            }));
        }), {
        ...
        if (counter.length > 0) counter.forEach((item => {
            if (item.hasAttribute("data-counter")) {
                const counterPercent = item.querySelector(".counter__percent");
                let observer = new IntersectionObserver((function(entries) {
                    entries.forEach((function(entry) {
                        if (entry.isIntersecting) {
                            const counters = item.querySelectorAll(".anim-number");
                            const speed = 600;
                            counterPercent.classList.add("counter__percent--active");
